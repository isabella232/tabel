...
def _IsStaging():
  return app_identity.get_application_id().endswith('-stage')

def _IsProduction():
  return app_identity.get_application_id().endswith('-prod')

def _AppId():
    return app_identity.get_application_id()

IS_DEV = _IsDevAppServer()
IS_STAGING = _IsStaging()
IS_PRODUCTION = _IsProduction() and not _IsDevAppServer()
